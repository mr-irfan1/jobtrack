...
function AchievementFormModal({
  ...
  return (
    <div
      ...
      className="fixed inset-0 z-50 flex items-center justify-center bg-background/80 p-3 sm:p-5 backdrop-blur-xs"
    >
      <div className="flex max-h-[calc(100dvh-2rem)] w-full max-w-lg flex-col rounded-2xl border border-border bg-surface p-5 sm:p-6 shadow-xl overflow-hidden">
        <div className="flex shrink-0 items-center justify-between border-b border-border pb-4">
          <h2 id="achievement-modal-title" className="text-lg font-bold text-foreground">
        ...
        {error ? (
          <div className="mt-3 rounded-xl border border-danger/30 bg-danger/10 p-3 text-xs font-semibold text-danger-fg shrink-0">
            {error}
          </div>
        ) : null}
        ...
        <form onSubmit={handleSubmit} className="mt-4 flex flex-1 flex-col overflow-hidden min-h-0">
          ...
          {/* ACTIONS */}
          <div className="mt-4 flex shrink-0 flex-wrap justify-end gap-3 pt-3 border-t border-border">
            <button
              type="button"
              onClick={onCancel}
